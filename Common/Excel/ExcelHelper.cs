...
namespace Common.Excel
{
    ...
    public static class ExcelHelper
    {
        ...
        private const String ext2007m = ".xlsm";

        //关键行值集合
        private static List<String> KeyRowContents = new List<String> { "*", "c", "s" };
        ...
        public static IWorkbook Load(String path)
        {
            ...
            if (extension == ext2007 || extension == ext2007m)
        ...
        private static Int32 ExactColumnNum(this ISheet sheet)
        {
            ...
            for (int i = 0; i < refRow.Cells.Count; i++)
            {
                String value = GetCellValue(refRow.GetCell(i)).ToString();

                if ((refRow.Cells[i].CellType == CellType.Blank || value == String.Empty) && i != 0)
                    return i;
            }

            return refRow.Cells.Count;
        }
        ...
        private static Int32 ExactRowNum(this ISheet sheet)
        {
            ...
            for (Int32 i = refRowNum; i >= 0; i--)
            {
                ...
                if (GetCellValue(row.GetCell(KeyColumnIndex)).ToString() == String.Empty)
                    continue;
